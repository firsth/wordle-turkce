...
function gridGuncelle(){
    for(let i=0; i<state.grid.length; i++){
        for(let j=0; j<state.grid[i].length; j++){
            const kutu = document.getElementById(`kutu${i}${j}`);
            kutu.textContent = state.grid[i][j];
        }
    }
}

function KutuOlustur(container,satir,sutun,letter = ''){
    const kutu = document.createElement('div');
    kutu.className = 'kutu';
    kutu.id = `kutu${satir}${sutun}`;
    kutu.textContent = letter;

    container.appendChild(kutu);
    return kutu;
}

function GridOlustur(container){
    const grid = document.createElement('div');
    grid.className = 'grid';

    for(let i = 0; i < 6; i++){
        for(let j = 0; j < 5; j++){
            KutuOlustur(grid,i,j);
        }
    }
...
function kelimeKontrol(kelime){
    return kelimeler.includes(kelime)
}
...
function yerKontrol(tahmin){
    ...
    const kazanmaKontrol = state.secret === tahmin;
...
function harfKontrol(harf){
    return harf.length === 1 && harf.match(/[a-z-ğüşöçıİĞÜŞÖÇ]/i);
}
...
function tusSil(){
    ...
    state.grid[state.satirSayisi][state.sutunSayisi - 1] = '';
    state.sutunSayisi--;
}
